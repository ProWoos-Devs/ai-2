...
4. Only for a candidate that clears the gate, and after Rafael has ruled on the LFM license (commercial use capped at 10 million USD annual revenue, not OSI), the catalog entry lands together with two new optional fields: `active_params_b` (feeds the speed estimate) and `class_b` (the dense-equivalent quality class, feeds the ranking that `params_b` does today). Never before, a key nothing reads is the `gpu_offload` mistake again.
