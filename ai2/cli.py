...
def cmd_model_pull(args) -> int:
    hw = detect()
    if args.model:
        model = runner._catalog_entry(args.model)
        if model is None:
        ...
        if model is None:
            ...
            return 1
    return runner._pull_model(model, force=args.force)
...
def cmd_model_list(args) -> int:
    ...
    others += [m for m in runner._speech_entries() if not any(s["id"] == m["id"] for s in speech)]
    if others:
...
def cmd_serve(args) -> int:
    ...
    if embedding:
        ...
        idle_arg = args.idle_timeout if args.idle_timeout is not None else runner.EMBED_IDLE_OR(defaults)
    else:
...
def cmd_chat(args) -> int:
    ...
    if not runner._server_ready(url):
        ...
        last_note = start
        while time.monotonic() < deadline and not runner._server_ready(url):
            time.sleep(2)
            now = time.monotonic()
            if now - last_note >= 10:
                last_note = now
    ...
    running_model = runner._catalog_entry(running_id) if running_id else None
